...
const Welcome = () => {
  ...
  const { setPage: setOnboardingPage, setPopupVisible } = useOnboarding();
  ...
  useEffect(() => {
    intervalRef.current = setInterval(() => {
      const nextPage = (slideIndex + 1) % SPLASH_SCREEN_INFO.length;
      setSlideIndex(nextPage);
  ...
  return (
    <TouchableWithoutFeedback onPress={() => Keyboard.dismiss()} accessible={false}>
      <Box backgroundColor="pearl" flex={1} justifyContent="center" alignItems="center">
            ...
            <Box flexDirection="row" width="100%" justifyContent="center" alignItems="center">
              {SPLASH_SCREEN_INFO.map((_, index) => (
                <Box
                  ...
                  borderRadius="s"
                  margin="xs"
                  backgroundColor={index === slideIndex ? "ink" : "honey"}
                />
              ))}
            </Box>
          </AnimatedBox>
        ...
        <LoginModal ref={loginRef} onClose={() => setPopupVisible(false)} />
      </Box>
